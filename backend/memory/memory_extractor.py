...
def extract_user_fact(text: str) -> Optional[Dict]:
    ...
    for pattern in [
        ...
        if m:
            ...
            if len(loc) > 1:
                return _fact(
                    f"Lives in {loc.title()}", "location", "location", loc.title(), 0.9
                )
    ...
    if m:
        color = m.group(2)
        return _fact(
            f"Favorite color is {color}", "preference", "favorite_color", color, 0.9
        )

    # ── FAVORITE FOOD ─────────────────────────────────────
    m = re.search(
        r"(my (?:fav(?:ou?rite)?) food is|i love eating)\s+([A-Za-z\s]+?)(?:\.|,|$)", t
    )
    if m:
        food = m.group(2).strip()
        return _fact(
            f"Favorite food is {food}", "preference", "favorite_food", food, 0.9
        )
    ...
    for pattern in project_patterns:
        ...
        if m:
            ...
            if len(project) > 3:
                return _fact(
                    f"Working on: {project}", "project", "active_project", project, 0.85
                )
    ...
    if m:
        ...
        return _fact(
            f"Favorite {pref_type} is {pref_val}",
            "preference",
            f"favorite_{pref_type}",
            pref_val,
            0.85,
        )
    ...
    # Hobby extraction
    _hm = re.search(
        r"i (?:also )?(?:love|enjoy|like|adore) (?:playing |doing |watching )?([A-Za-z][A-Za-z ]{2,25}?)(?:\.|,|$|!)",
        t,
    )
    if _hm:
