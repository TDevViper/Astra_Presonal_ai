...
try:
    ...
    _dbg(
        ...
        {"repo_id": "hexgrad/Kokoro-82M", "modelPathExists": os.path.exists(_MODEL)},
    )
    # endregion agent log
    try:
        ...
        _dbg(
            "H6-kokoro-offline",
            "Kokoro init failed with repo_id; retrying without repo_id",
    ...
    _dbg(
        "H4-kokoro-init",
        "Kokoro init failed (falling back)",
        {"errorType": type(e).__name__, "error": str(e)[:200]},
    )
    # endregion agent log
    ...
    def speak(self, text: str, voice: str = "af_bella", speed: float = 1.1):
        if not KOKORO_AVAILABLE:
            ...
            (
                ...
                else print(f"[TTS fallback] {text[:80]}")
            )
            return
        try:
            import numpy as np
