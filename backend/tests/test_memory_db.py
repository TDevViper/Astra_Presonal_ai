...
def tmp_db(tmp_path, monkeypatch):
    ...
    # Force-close any cached connection and wipe thread-local state
    try:
        if getattr(memory_db._local, "conn", None):
            memory_db._local.conn.close()
    except Exception:
        pass
    memory_db._local.__dict__.clear()
    memory_db.init_db()
    yield db
    try:
        if getattr(memory_db._local, "conn", None):
...
cat > backend/tests/test_memory_db.py << 'EOF'
"""Tests for memory_db — WAL mode, save/load, facts."""
import os
import sys
import pytest
