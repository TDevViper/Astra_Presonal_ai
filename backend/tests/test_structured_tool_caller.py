...
def _make_stc():
    mm = MagicMock()
    te = MagicMock()
    br = MagicMock(return_value={"reply": "ok", "intent": "tool_git", "agent": "tool/git", "confidence": 0.92})
...
def test_llm_no_tool_call_returns_none():
    stc = _make_stc()
    fake_resp = {"message": {"content": "I don't need a tool for that.", "tool_calls": []}}
    with patch("tools.tool_schemas.get_schemas_for_model", return_value=[{"type": "function", "function": {"name": "git"}}]):
        with patch("ollama.Client") as mock_client:
            mock_client.return_value.chat.return_value = fake_resp
            result = stc.try_tool_call("hello how are you", "sys", "llama3.2", [], {}, "User")
    assert result is None
...
def test_llm_tool_call_triggers_execution():
    ...
    fake_resp = {
        "message": {
            "tool_calls": [{"function": {"name": "system_monitor", "arguments": {"detail": "summary"}}}]
        }
    }
    with patch("tools.tool_schemas.get_schemas_for_model", return_value=[{"type":"function","function":{"name":"system_monitor"}}]):
        with patch("ollama.Client") as mock_client:
            mock_client.return_value.chat.return_value = fake_resp
            with patch.object(stc, "_execute_tool", return_value="CPU: 45%") as mock_exec:
                with patch.object(stc, "_synthesize_reply", return_value="Your CPU is at 45%."):
                    result = stc.try_tool_call("how's my cpu", "sys", "llama3.2", [], {}, "User")
    mock_exec.assert_called_once_with("system_monitor", {"detail": "summary"}, "how's my cpu", {}, "User")
...
def test_args_as_json_string_parsed_correctly():
    ...
    fake_resp = {
        "message": {
            "tool_calls": [{"function": {"name": "git", "arguments": json.dumps({"operation": "status"})}}]
        }
    }
    with patch("tools.tool_schemas.get_schemas_for_model", return_value=[{"type":"function","function":{"name":"git"}}]):
        with patch("ollama.Client") as mock_client:
            ...
            with patch.object(stc, "_execute_tool", return_value="clean") as mock_exec:
                with patch.object(stc, "_synthesize_reply", return_value="Repo is clean."):
                    stc.try_tool_call("git status", "sys", "llama3.2", [], {}, "User")
    # args should be parsed from JSON string to dict
    mock_exec.assert_called_once_with("git", {"operation": "status"}, "git status", {}, "User")
...
def test_synthesis_failure_returns_raw_tool_result():
    ...
    with patch("ollama.Client") as mock_client:
        ...
        with patch("config.config") as mock_cfg:
            ...
            result = stc._synthesize_reply("q", "git", "CPU: 80%", "sys", "llama3.2", [])
