...
class SmartHome:
    ...
    def lock_door(self, entity_id: str):
        requests.post(
            f"{HASS_URL}/api/services/lock/lock",
            headers=self.headers,
    ...
    def unlock_door(self, entity_id: str):
        requests.post(
            f"{HASS_URL}/api/services/lock/unlock",
            headers=self.headers,
            json={"entity_id": entity_id},
            timeout=5,
        )
    ...
    def get_camera_feed(self, camera_entity: str):
        r = requests.get(
            f"{HASS_URL}/api/camera_proxy/{camera_entity}",
            headers=self.headers,
            timeout=10,
        )
        return r.content
    ...
    def run_automation(self, automation_id: str):
        requests.post(
            f"{HASS_URL}/api/services/automation/trigger",
            headers=self.headers,
    ...
    def get_sensor(self, entity_id: str):
        r = requests.get(
            f"{HASS_URL}/api/states/{entity_id}", headers=self.headers, timeout=5
        )
        return r.json()
