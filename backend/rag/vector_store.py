...
def _load_index():
    ...
    return faiss.IndexFlatIP(
        DIM
    )  # Inner product for normalized vecs = cosine similarity
...
def add_chunks(
    chunks: list[str], embeddings: np.ndarray, source: str, tags: list[str] = None
):
    index = _load_index()
    meta = _load_meta()
...
def search(query_vec: np.ndarray, top_k: int = 5) -> list[dict]:
    ...
    scores, indices = index.search(
        query_vec.reshape(1, -1).astype(np.float32), min(top_k, index.ntotal)
    )
    results = []
