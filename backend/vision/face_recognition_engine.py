...
def learn_face(name: str, image_b64: str) -> Dict:
    ...
    try:
        ...
        return {
            "success": True,
            "name": name,
            "message": f"Got it — I'll recognize {name} from now on.",
        }

    except ImportError:
        return {
            "success": False,
            "error": "face_recognition library not installed. Run: pip install face-recognition",
        }
    except Exception as e:
...
def identify_faces(image_b64: str, tolerance: float = 0.55) -> Dict:
    ...
    try:
        ...
        if identified:
            names = ", ".join(
                f"{r['name']} ({int(r['confidence'] * 100)}%)" for r in identified
            )
            if unknown > 0:
