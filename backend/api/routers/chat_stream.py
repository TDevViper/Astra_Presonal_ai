    ...
    def _run_brain():
        ...
        try:
            ...
            loop.call_soon_threadsafe(queue.put_nowait,
                json.dumps({"type": "meta", "model": selected_model, "intent": query_intent}))
            ...
            for chunk in brain.process_stream(user_input):
                ...
                if token:
                    try:
                        loop.call_soon_threadsafe(queue.put_nowait,
                            json.dumps({"type": "token", "text": token}, ensure_ascii=False))
            ...
            try:
                loop.call_soon_threadsafe(queue.put_nowait,
                    json.dumps({"type": "error", "message": "Stream processing failed"}))
            except Exception:
                pass
        finally:
