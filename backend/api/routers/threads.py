...
from memory.threads_db import (
    init_db, create_thread, get_threads, get_thread,
    get_messages, add_message, rename_thread, archive_thread, fork_thread
)
...
def send(thread_id: str, body: SendMessage,
         current_user=Depends(require_permission("chat"))):
    thread = get_thread(thread_id, current_user["id"])
    if not thread:
        raise HTTPException(status_code=404, detail="Thread not found")
    ...
    if body.use_ai:
        try:
            ...
            history = [
                ...
                for m in get_messages(thread_id, limit=20)[:-1]  # exclude just-added msg
            ]
            brain  = get_brain()
            result = brain.process(body.content, history=history,
...
def rename(thread_id: str, body: RenameThread,
           current_user=Depends(require_permission("chat"))):
...
def fork(thread_id: str, body: ForkThread,
         current_user=Depends(require_permission("chat"))):
    thread = get_thread(thread_id, current_user["id"])
    if not thread:
        raise HTTPException(status_code=404, detail="Thread not found")
