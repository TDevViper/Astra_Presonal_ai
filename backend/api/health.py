...
from core.smart_guardian import (
    get_full_stats as _smart_stats,
    get_trend_summary as _trend_summary,
)
...
def health():
    ...
    try:
        ...
        welcome = get_welcome_back(
            user_name=_mem_h.get("preferences", {}).get("name", "User")
        )
        if welcome:
    ...
    return JSONResponse(
        content=status, status_code=200 if status["status"] == "ok" else 207
    )
...
def _check_voice() -> dict:
    try:
        ...
        tts_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "tts_kokoro.py"
        )
