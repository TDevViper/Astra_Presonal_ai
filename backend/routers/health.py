...
async def health():
    from api.health import (
        _check_ollama, _check_memory, _check_vectors,
        _check_voice, _check_redis, _check_disk,
        _check_guardian, _check_plugins
    )
    ...
    try:
        ...
        mem     = load_memory()
        welcome = get_welcome_back(user_name=mem.get("preferences", {}).get("name", "User"))
        if welcome:
