...
def get_self_response(
    query: str, user_name: str = "Arnav", capabilities_enabled: Dict = None
) -> str:
    """
    ...
    if any(
        p in t
        for p in ["who are you", "what are you?", "what is astra", "are you an ai"]
    ) or t.strip() in ["what are you", "what are you."]:
        return (
    ...
    if any(
        p in t
        for p in ["what can you do", "capabilities", "what do you know how to do"]
    ):
        active = [f"• {name}: {desc}" for name, desc in ASTRA_CAPABILITIES.items()]
        return f"Here's what I can do, {creator}:\n" + "\n".join(active)

    # Limitations
    if any(
        p in t for p in ["limitation", "what can't you", "what cannot", "limitations"]
    ):
