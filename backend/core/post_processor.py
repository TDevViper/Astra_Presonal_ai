...
class PostProcessor:
    ...
    def process(
        ...
        for step in [
            lambda r: self._critic(
                r, user_name, memory, user_input, selected_model, query_intent
            ),
            ...
            lambda r: self._emotion_prefix(
                r, emotion_label, emotion_score, user_name, memory
            ),
            ...
            try:
                ...
                logger.warning("post_processor step failed: %s", e)
        return reply

    def _critic(self, r, un, m, ui, model, intent):
        try:
            return critic_review(r, un, m, user_input=ui, model=model, intent=intent)
        except Exception as e:
    ...
    def _truth_guard(self, r):
        try:
            ...
            if not ok:
                logger.warning("TruthGuard violation: %s", v)
            return self._tg.get_safe_reply(v)
        except Exception as e:
    ...
    def _limit(self, r, ui):
        try:
            return limit_words(r, intent=detect_intent_for_limit(ui))
        except Exception as e:
    ...
    def _emotion_prefix(self, r, label, score, un, m):
        if score > 0.7 and label in {"sad", "angry", "anxious", "tired"}:
            try:
                return f"{emotion_reply(label, score, un, m)} {r}"
            except Exception as e:
