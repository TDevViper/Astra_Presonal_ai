...
def classify_intent(text: str) -> str:
    t = text.lower()
    if any(w in t for w in ["turn on","turn off","light","lock"]):   return "smart_home"
    if any(w in t for w in ["weather","news","search","latest"]):    return "search"
    if any(w in t for w in ["code","python","debug","error","bug"]): return "coding"
    if any(w in t for w in ["remember","memory","recall"]):          return "memory"
    if any(w in t for w in ["camera","screen","see","vision"]):      return "vision"
    if any(w in t for w in ["why","how","explain","analyze"]):       return "reasoning"
    return "general"
...
def _score_reply(user_input: str, reply: str, intent: str) -> float:
    score = 0.5
    filler_starters = ["sure!","certainly!","of course!","great question","absolutely!","i'd be happy","i'm happy to"]
    if any(reply.lower().startswith(f) for f in filler_starters): score -= 0.2
    words = len(reply.split())
    if intent in ["reasoning","coding"] and words < 30:  score -= 0.15
    if intent == "general" and words > 150:              score -= 0.1
    if 20 <= words <= 200:                               score += 0.1
    unique_r = len(set(reply.lower().split())) / max(len(reply.split()), 1)
    if unique_r < 0.5:                                   score -= 0.15
    if intent == "coding" and "```" in reply:            score += 0.15
    deflections = ["i don't know","i'm not sure","i cannot","as an ai"]
    if any(d in reply.lower() for d in deflections) and intent == "general": score -= 0.1
...
def generate_report() -> str:
    ...
    tips  = _load_tips()
    if not spots: return "No response data yet."
...
def auto_refine_prompts() -> str:
    try:
        ...
        tips = _load_tips()
        if not weak: return "All intents performing well."
        summary = "\n".join([f"- {i} ({v:.0%}): {'; '.join(tips.get(i,['no specific issues']))}"
                             for i,v in weak.items()])
        resp = ollama.chat(model="phi3:mini",
                           messages=[{"role":"user","content":
                               f"AI assistant has these issues:\n{summary}\n"
...
class SelfImprovementEngine:
    def __init__(self):
        os.makedirs(os.path.dirname(LOG_FILE), exist_ok=True)
    def log_response(self, u, r, c, rating=None): log_response(u, r, c, rating)
    def analyze_weak_spots(self):                 return analyze_weak_spots()
    def generate_report(self):                    return generate_report()
    def auto_refine_prompts(self):                return auto_refine_prompts()
