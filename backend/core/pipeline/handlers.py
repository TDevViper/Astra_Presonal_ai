...
class ModeSwitchHandler(Handler):
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if mode:
                set_mode(mode)
                return Reply(text=get_mode_banner(), intent="mode_switch",
                             agent="system", confidence=1.0)
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if chain:
                ...
                if reply:
                    return Reply(text=reply, intent="chain",
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if result:
                ...
                return Reply(text=reply_text, intent=intent,
                             agent=agent, confidence=1.0, tool_used=True)
        except Exception as e:
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        ...
        try:
            ...
            if shortcut:
                ...
                r = Reply(text=shortcut, intent="shortcut",
                          agent="intent_handler",
                          confidence=conf_score("shortcut", "shortcut"))
                self._cache.set(ctx.user_input, r.to_dict(ctx.emotion_label), ctx.session_id)
                return r
        except Exception as e:
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if is_self_query(ctx.user_input):
                ...
                return Reply(text=reply, intent="self_awareness",
                             agent="self", confidence=1.0)
        except Exception as e:
...
class ToolDispatchHandler(Handler):
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if compound:
                return Reply(text=compound, intent="shortcut",
                             agent="system_controller", confidence=1.0,
                             tool_used=True)
            tool = detect_tool(ctx.user_input)
            if tool and self._caps.is_enabled(tool):
                result = self._tools.execute(tool, ctx.user_input,
                                             ctx.memory, ctx.user_name)
                if result:
                    return Reply(
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        try:
            ...
            if fact:
                ...
                if not is_question_like(ctx.user_input):
                    return Reply(
                        ...
                        intent="memory_storage", agent="memory",
            ...
            if recalled:
                self._mem.save(ctx.memory)
                return Reply(
                    text=recalled, intent="memory_recall", agent="memory",
...
class WebSearchHandler(Handler):
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        ...
        try:
            ...
            return Reply(
                text=result["reply"], intent="web_search",
                agent="web_search_agent", confidence=conf_score("web_search_agent", "web_search"),
...
class LLMHandler(Handler):
    ...
    def handle(self, ctx: RequestContext) -> Optional[Reply]:
        from core.confidence import score as conf_score
        try:
            query_intent   = ctx.query_intent or self._mm.classify_query_intent(ctx.user_input)
            selected_model = ctx.selected_model or self._mm.select_model(ctx.user_input, query_intent)

            system_prompt, sem_conf = self._ctx.build(
                ctx.user_input, ctx.user_name, ctx.memory,
                ctx.emotion_label, query_intent, ctx.history
            )
            ...
            if ctx.streaming:
                ...
                return Reply(
                    text="", intent=query_intent,
                    agent=f"ollama/{selected_model}",
                    confidence=max(conf_score(f"ollama/{selected_model}", query_intent), sem_conf),
            ...
            # Blocking LLM call
            reply = self._llm.try_react(ctx.user_input, selected_model,
                                        system_prompt, ctx.user_name)
            if not reply:
                reply = self._llm.call(ctx.user_input, system_prompt,
                                       selected_model, query_intent, ctx.history)

            final_conf = max(conf_score(f"ollama/{selected_model}", query_intent), sem_conf)
            return Reply(
                text=reply, intent=query_intent,
                agent=f"ollama/{selected_model}", confidence=final_conf,
            )

        except Exception as e:
            logger.error("LLMHandler: %s", e, exc_info=True)
            return Reply(text="Something went wrong.", intent="error",
                         agent="error_handler", confidence=0.0)
