...
def run_auto_debug(error_text: str, user_name: str = "User",
                   model: str = "phi3:mini") -> Dict:
    """
    ...
    results = {"error": error_text, "reply": "", "fix_proposed": False,
               "commit_proposal": None, "search_results": None}
    ...
    try:
        ...
        results["reply"] = (f"Found this error: **{clean_error}**\n\n"
                            f"{results['search_results'][:400]}")
...
def _looks_like_code_error(error: str) -> bool:
    code_indicators = ["Error:", "Exception", "Traceback", "undefined",
                       "cannot find module", "import error", "syntax error",
                       "is not defined", "has no attribute"]
