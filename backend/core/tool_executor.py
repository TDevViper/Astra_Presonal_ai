...
class ToolExecutor:
    ...
    def execute(
        self, tool: str, user_input: str, memory: Dict, user_name: str
    ) -> Optional[Dict]:
    ...
    def _file_reader(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        if not filepath:
            if any(
                w in user_input.lower() for w in ["list", "show files", "what files"]
            ):
                result = list_files()
                if result["success"]:
    ...
    def _task_manager(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        if any(w in text for w in ["add task", "new task", "remind me", "todo"]):
            ...
            reply = (
                f"✓ {tm.add_task(m.group(1).strip())['message']}" if m else "What task?"
            )
    ...
    def _git(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        if not is_git_repo():
            return self._build_reply(
                "Not in a git repo.", "neutral", "git_error", "git", confidence=1.0
            )
        ...
        if "status" in text or "what changed" in text:
            ...
            if result["success"]:
                if result.get("clean"):
                    ...
                    if result.get("modified"):
                        reply += f"Modified: {', '.join(result['modified'])}\n"
                    if result.get("added"):
                        reply += f"Added: {', '.join(result['added'])}\n"
                    if result.get("untracked"):
                        reply += f"Untracked: {', '.join(result['untracked'])}\n"
            else:
                reply = f"Error: {result['error']}"
            ...
            return {
                "reply": "Git commit proposed",
                "emotion": "neutral",
                "intent": "git_commit_proposal",
                "agent": "git",
                "tool_used": True,
                "confidence": 1.0,
        ...
        return self._build_reply(
            reply, "neutral", "git_operation", "git", tool_used=True, confidence=0.90
        )
    ...
    def _python_sandbox(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        if not code:
            return self._build_reply(
                "No Python code found. Wrap in ```python``` blocks.",
                "neutral",
                "python_error",
                "python_sandbox",
                confidence=0.80,
        ...
        return {
            "reply": "Python execution proposed",
            "emotion": "neutral",
            "intent": "python_execution_proposal",
            "agent": "python_sandbox",
            "tool_used": True,
            "confidence": 1.0,
            "approval_required": True,
    ...
    def _system_controller(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        return self._build_reply(
            "I couldn't execute that system command.",
            "neutral",
            "system_control",
            "system_controller",
            tool_used=True,
    ...
    def _face_recognition(self, user_input: str, memory: Dict, user_name: str) -> Dict:
        ...
        if learn_match and any(
            w in text for w in ["remember", "learn", "this is", "name is"]
        ):
        ...
        # List known faces
        if any(
            w in text for w in ["list", "who do you know", "known faces", "my faces"]
        ):
