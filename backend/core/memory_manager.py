...
class MemoryManager:
    ...
    def extract_and_store_fact(
        self, user_input: str, memory: Dict, user_name: str
        ...
        try:
            ...
            if fact:
                ...
                existing = [
                    f
                    for f in memory.get("user_facts", [])
                    if f.get("subtype") == fact.get("subtype")
                    and str(f.get("value", "")).lower()
                ...
                is_dup = any(
                    e.get("subtype") == fact.get("subtype")
                    and str(e.get("value", "")).lower()
                    == str(fact.get("value", "")).lower()
                    for e in existing
                )
                if not is_dup:
        ...
        try:
            ...
            if should_summarize(history):
                summary = summarize_conversation(
                    history, memory, user_name, model=selected_model
                )
