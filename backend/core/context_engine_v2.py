...
def select_best_chunks(
    ...
    for chunk in scored[: top_k * 2]:
        ...
        if len(selected) >= top_k:
            break
    logger.info(
        "context_v2: selected %d/%d chunks (%d tokens)",
        len(selected),
        len(chunks),
        used_tokens,
    )
...
def build_ranked_context(
    query: str, user_name: str, query_intent: str = "general"
) -> Tuple[str, float]:
    t0 = time.time()
    try:
    ...
    if exchanges:
        ranked_ex = select_best_chunks(
            query, exchanges, max_tokens=300, top_k=MAX_EXCHANGES
        )
        if ranked_ex:
    ...
    logger.info(
        "context_v2: built in %dms | boost=%.2f | chars=%d",
        elapsed_ms,
        confidence_boost,
        len(context),
    )
