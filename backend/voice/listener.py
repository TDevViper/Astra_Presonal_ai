        ...
        try:
            ...
            try:
                ...
                _dbg(
                    ...
                    {"errorType": type(e).__name__, "error": str(e)[:200]},
                )
                # endregion agent log
                raise
...
def vad_trim(
    audio: np.ndarray, sample_rate: int = 16000, threshold: float = 0.008
) -> np.ndarray:
    """Trim leading and trailing silence."""
    frame_len = int(sample_rate * 0.02)
    frames = [
        audio[i : i + frame_len] for i in range(0, len(audio) - frame_len, frame_len)
    ]
    speech = [
        i for i, f in enumerate(frames) if float(np.sqrt(np.mean(f**2))) >= threshold
    ]
    if not speech:
        return audio
